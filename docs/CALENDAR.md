...
A migration `20260925120000_calendar_days.sql` cria `public.calendar_days` com uma linha por `(user_id, date)`, restrição dos dois estados, índice por usuário/data, políticas RLS de proprietário e permissões explícitas ao papel `authenticated`. No projeto Supabase hospedado `Percorso` (`rkkilslsrpdvvijnpulq`), esta migration foi executada em 25/09/2026 no SQL Editor, dentro de uma transação, pois o banco não possuía histórico em `supabase_migrations.schema_migrations`. A verificação retornou `calendar_days`, RLS ativo, quatro políticas e permissões de SELECT, INSERT, UPDATE e DELETE para `authenticated`. **Não reaplique esta migration nesse banco** com `db push` sem antes reconciliar o histórico de migrations; em outros ambientes, aplique pelo fluxo normal de migrations. `supabase/schema-full.sql` é apenas um retrato destrutivo para bancos descartáveis: **não o execute no banco de produção**. As seeds existentes não foram alteradas nesta branch; após aplicar a migration, marque dias manualmente na conta de teste para validar a tela.
